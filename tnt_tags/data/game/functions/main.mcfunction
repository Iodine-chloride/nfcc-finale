...
execute if score EVENT_STARTED data matches 1 if score PLAYED_TNT_TAGS data matches 0..2 if score STARTED data matches 0 if score INGAME data matches 0 run item replace entity @a[scores={ready=0}] hotbar.4 with minecraft:carrot_on_a_stick{Ready:1b,Unthrowable:1b,display:{Name:'[{"text":"§a§l准备"},{"text":" - "},{"text":"§7右键使用（无法撤销）"}]'}}
clear @a[scores={ready=1}]

# Getting ready
execute as @a[scores={carrot_stick=1,ready=0}] run tellraw @a [{"text":"[","color":"white"},{"text":"✔","color":"green"},{"text":"] "},{"selector":"@s","color":"gold"},{"text":" 戴上了炸弹人面罩！"}]
...
execute if score CHOSEN_MAP data matches 601 unless entity @a[tag=!spec,scores={ready=0}] run title @a subtitle [{"text":"地图：","color":"yellow"},{"text":"Grand Mansions","color":"gold"}]
execute if score CHOSEN_MAP data matches 602 unless entity @a[tag=!spec,scores={ready=0}] run title @a subtitle [{"text":"地图：","color":"yellow"},{"text":"Into the Hell","color":"gold"}]
