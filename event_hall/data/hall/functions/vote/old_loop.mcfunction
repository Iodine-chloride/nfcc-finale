...
execute if score PLAYED_bingo data matches 0 run item replace entity @a[tag=!spec,scores={carrot_stick=0}] hotbar.5 with minecraft:snowball{Tags:["bingo"],display:{Name:'[{"text":"Murder Mystery","color":"light_purple","italic":false,"bold":true},{"text":" - ","color":"white"},{"text":"Throw Me!","color":"gray","bold":false}]'}} 9
...
execute unless score PLAYED_bingo data matches 0 run item replace entity @a[tag=!spec,scores={carrot_stick=0}] hotbar.5 with minecraft:barrier{display:{Name:'{"text":"该游戏已被选中！","color":"red","italic":false,"bold":true}'}}
