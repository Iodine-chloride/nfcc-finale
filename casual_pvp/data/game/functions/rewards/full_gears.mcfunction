...
execute if score EVENT_STARTED data matches 0 run item replace entity @a hotbar.1 with minecraft:iron_axe{Unbreakable:1b}
execute if score EVENT_STARTED data matches 0 run item replace entity @a hotbar.2 with minecraft:bow{Unbreakable:1b,Enchantments:[{id:"minecraft:infinity",lvl:1}]}
execute if score EVENT_STARTED data matches 0 run item replace entity @a weapon.offhand with minecraft:shield{Unbreakable:1b}
...
execute if score EVENT_STARTED data matches 0 run tellraw @a [{"text":"PVP mode is now switched to ","color":"yellow"},{"text":"Full Gears","color":"green"},"!"]

execute if score EVENT_STARTED data matches 1 run tellraw @a [{"text":"[⚠] "},{"text":"似乎有人试图改变游戏规则...但现在是不允许的 :)","color":"red"}]

advancement revoke @a only game:full_gears