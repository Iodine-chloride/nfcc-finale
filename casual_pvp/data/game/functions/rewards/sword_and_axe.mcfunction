...
execute if score EVENT_STARTED data matches 0 run tellraw @a [{"text":"PVP mode is now switched to ","color":"yellow"},{"text":"Sword and Axe","color":"green"},"!"]

execute if score EVENT_STARTED data matches 1 run tellraw @a [{"text":"[⚠] "},{"text":"似乎有人试图改变游戏规则...但现在是不允许的 :)","color":"red"}]

advancement revoke @a only game:sword_and_axe