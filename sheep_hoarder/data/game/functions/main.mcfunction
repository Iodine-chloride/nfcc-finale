...
execute if score CHOSEN_MAP data matches 501 unless entity @a[tag=!spec,scores={ready=0}] run title @a subtitle [{"text":"地图：","color":"yellow"},{"text":"Aztec","color":"gold"}]
execute if score CHOSEN_MAP data matches 502 unless entity @a[tag=!spec,scores={ready=0}] run title @a subtitle [{"text":"地图：","color":"yellow"},{"text":"Thorn's Arena","color":"gold"}]
