...
execute as @e[tag=waiting_room_sh,limit=1] at @s run tp @a @s
...
# Event settings
execute if score EVENT_STARTED data matches 1 run title @a title {"text":"Sheep Hoarders","color":"yellow"}
execute if score EVENT_STARTED data matches 1 run title @a subtitle [{"text":"Games: ","color":"gold"},{"score":{"name":"PLAYED_SHEEP_HOARDERS","objective":"data"},"color":"green"},{"text":"/2","color":"green"}]
execute if score EVENT_STARTED data matches 1 if score PLAYED_SHEEP_HOARDERS data matches 2.. run tellraw @a [{"text":"[⭐] "},{"text":"已完成游戏","color":"yellow"},{"text":"Sheep Hoarders","color":"green","bold":true},{"text":"的对决！10秒后将被传送回联赛大厅。","color":"yellow"}]
execute if score EVENT_STARTED data matches 1 if score PLAYED_SHEEP_HOARDERS data matches 2.. run schedule function game:general/return_hall 10s